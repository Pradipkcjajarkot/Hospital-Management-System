  ...
  return (
    <div>
      <div className="mb-6 flex items-center gap-2">
        <button onClick={() => setTab('listings')} className={`rounded-xl px-5 py-2.5 text-sm font-medium transition-all ${tab === 'listings' ? 'bg-rose-600 text-white shadow-sm' : 'bg-gray-100 text-gray-600 hover:bg-gray-200 dark:bg-gray-700 dark:text-gray-300'}`}><Briefcase className="mr-1.5 inline h-4 w-4" />{t('careerManagement')}</button>
        <button onClick={() => setTab('applications')} className={`rounded-xl px-5 py-2.5 text-sm font-medium transition-all ${tab === 'applications' ? 'bg-rose-600 text-white shadow-sm' : 'bg-gray-100 text-gray-600 hover:bg-gray-200 dark:bg-gray-700 dark:text-gray-300'}`}><Send className="mr-1.5 inline h-4 w-4" />{t('applications')} ({applications.length})</button>
      </div>
    ...
    return (
      <form onSubmit={handleSave} className="space-y-4">
        <div className="grid gap-4 sm:grid-cols-2">
          <div className="sm:col-span-2"><label className="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">{t('jobTitle')} *</label><input required value={form.title} onChange={e => setForm(f => ({ ...f, title: e.target.value }))} className="w-full rounded-xl border border-gray-300 px-4 py-3 text-sm focus:border-rose-500 focus:ring-2 focus:ring-rose-200 dark:border-gray-600 dark:bg-gray-700 dark:text-white" /></div>
          <div><label className="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">{t('jobDepartment')}</label><input value={form.department} onChange={e => setForm(f => ({ ...f, department: e.target.value }))} className="w-full rounded-xl border border-gray-300 px-4 py-3 text-sm focus:border-rose-500 focus:ring-2 focus:ring-rose-200 dark:border-gray-600 dark:bg-gray-700 dark:text-white" /></div>
          <div><label className="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">{t('jobLocation')}</label><input value={form.location} onChange={e => setForm(f => ({ ...f, location: e.target.value }))} className="w-full rounded-xl border border-gray-300 px-4 py-3 text-sm focus:border-rose-500 focus:ring-2 focus:ring-rose-200 dark:border-gray-600 dark:bg-gray-700 dark:text-white" /></div>
          <div><label className="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">{t('jobType')} *</label><select value={form.type} onChange={e => setForm(f => ({ ...f, type: e.target.value }))} className="w-full rounded-xl border border-gray-300 px-4 py-3 text-sm focus:border-rose-500 focus:ring-2 focus:ring-rose-200 dark:border-gray-600 dark:bg-gray-700 dark:text-white"><option value="full-time">{t('fullTime')}</option><option value="part-time">{t('partTime')}</option><option value="contract">{t('contract')}</option><option value="internship">Internship</option></select></div>
        ...
        <div className="flex items-center gap-3">
          ...
          <button type="button" onClick={() => { setEditing(null); setShowAdd(false) }} className="rounded-xl border border-gray-300 px-6 py-2.5 text-sm font-medium text-gray-700 hover:bg-gray-50 dark:border-gray-600 dark:text-gray-300">{t('cancel')}</button>
        </div>
      </form>
    )
  }
  ...
  return (
    <div>
      <div className="mb-4 flex items-center justify-between">
        <h2 className="text-lg font-bold text-gray-900 dark:text-white">{t('careerManagement')}</h2>
        <button onClick={() => { setShowAdd(true); setEditing(null) }} className="flex items-center gap-2 rounded-xl bg-rose-600 px-4 py-2 text-sm font-medium text-white hover:bg-rose-700 transition-all"><Plus className="h-4 w-4" /> {t('addJob')}</button>
      </div>
      {(showAdd || editing) && (
      ...
      <div className="space-y-3">
        {listings.length === 0 ? (
          <p className="py-8 text-center text-gray-500">{t('noJobs')}</p>
        ) : listings.map((l) => (
...
function ApplicationPanel({ applications, onRefresh, selected, setSelected }: {
  ...
  return (
    <div>
      <h2 className="mb-4 text-lg font-bold text-gray-900 dark:text-white">{t('applications')}</h2>
      <div className="space-y-3">
        {applications.length === 0 ? (
          <p className="py-8 text-center text-gray-500">{t('noApplications')}</p>
        ) : applications.map((a) => (
