  ...
  if (selected) {
    return (
      <div className="mx-auto max-w-3xl px-4 py-12">
        <button onClick={() => { setSelected(null); setShowApply(false) }} className="mb-4 flex items-center gap-2 text-sm text-gray-500 hover:text-rose-600"><ArrowLeft className="h-4 w-4" /> {t('backToJobs')}</button>
        {showApply ? (
          <ApplyForm job={selected} onDone={() => setShowApply(false)} />
        ) : (
          <>
            <div className="mb-6">
              <div className="flex items-center gap-2 text-xs font-medium uppercase tracking-wider text-rose-600 mb-2"><Building2 className="h-3.5 w-3.5" />{selected.department || t('general')}</div>
              <h1 className="text-3xl font-bold text-gray-900 dark:text-white mb-3">{selected.title}</h1>
              <div className="flex flex-wrap gap-3 text-sm text-gray-500">
                <span className="flex items-center gap-1"><MapPin className="h-4 w-4" />{selected.location || t('onSite')}</span>
            ...
            <div className="prose prose-sm max-w-none dark:prose-invert mb-8">
              ...
              {selected.requirements && <><h3 className="text-lg font-semibold text-gray-900 dark:text-white mt-6 mb-2">{t('requirements')}</h3><p className="text-gray-600 dark:text-gray-400 whitespace-pre-line">{selected.requirements}</p></>}
            </div>
            <button onClick={() => setShowApply(true)} className="rounded-xl bg-rose-600 px-8 py-3 text-sm font-medium text-white hover:bg-rose-700 transition-all">{t('applyNow')}</button>
          </>
        )}
      </div>
    )
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-12">
      <div className="mb-8 text-center">
        <h1 className="text-3xl font-bold text-gray-900 dark:text-white">{t('jobListings')}</h1>
        <p className="mt-2 text-gray-500 dark:text-gray-400">{t('careersSubtitle')}</p>
      </div>
      {loading ? (
        <div className="flex justify-center py-12"><Loader2 className="h-6 w-6 animate-spin text-rose-500" /></div>
      ) : listings.length === 0 ? (
        <p className="py-12 text-center text-gray-500">{t('noJobs')}</p>
      ) : (
            ...
            <button key={job.id} onClick={() => setSelected(job)} className="w-full rounded-2xl border border-gray-200 bg-white p-6 text-left shadow-sm transition-all hover:border-rose-200 hover:shadow-md dark:border-gray-700 dark:bg-gray-800 dark:hover:border-rose-700">
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1">
                  <div className="flex items-center gap-2 text-xs font-medium text-rose-600 uppercase tracking-wider mb-1"><Building2 className="h-3 w-3" />{job.department || t('general')}</div>
                  <h3 className="text-lg font-bold text-gray-900 dark:text-white">{job.title}</h3>
                  <div className="mt-2 flex flex-wrap gap-3 text-sm text-gray-500">
                    <span className="flex items-center gap-1"><MapPin className="h-3.5 w-3.5" />{job.location || t('onSite')}</span>
  ...
  if (submitted) {
    return (
      <div className="py-12 text-center">
        ...
        <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">{t('applicationSubmitted')}</h2>
        <p className="text-gray-500 mb-6">{t('applicationThankYou', { title: job.title })}</p>
        <button onClick={onDone} className="rounded-xl bg-gray-600 px-6 py-2.5 text-sm font-medium text-white hover:bg-gray-700 transition-all">{t('backToJob')}</button>
      </div>
    )
  }
    ...
    <div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <button disabled={submitting} className="rounded-xl bg-rose-600 px-8 py-3 text-sm font-medium text-white hover:bg-rose-700 disabled:opacity-50 transition-all flex items-center gap-2">
          {submitting ? <><Loader2 className="h-4 w-4 animate-spin" /> {t('submitting')}...</> : <><Send className="h-4 w-4" /> {t('send')}</>}
        </button>
      </form>
    </div>
  )
