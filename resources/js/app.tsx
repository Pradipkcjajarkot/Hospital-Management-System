...
function App() {
  ...
  if (page === 'login') {
    return <LoginPage onSignUp={() => setPage('signup')} onBack={() => setPage('landing')} />;
  }

  if (page === 'signup') {
    return <SignUpPage onLogin={() => setPage('login')} onBack={() => setPage('landing')} />;
  }
  ...
  return (
    <div className="flex min-h-screen flex-col bg-gradient-to-br from-blue-50 via-white to-red-50">
      ...
      <main className="flex flex-1 flex-col items-center justify-center px-4 py-12">
        <div className="max-w-2xl text-center">
          ...
          <button
            ...
            className="rounded-xl bg-red-600 px-10 py-3.5 text-lg font-medium text-white shadow-lg transition hover:bg-red-700"
          >
            Login to Dashboard
          </button>
        </div>
      ...
      <footer className="border-t bg-white py-4 text-center text-sm text-gray-400">
        &copy; 2026 MediCare Hospital Management System. All rights reserved.
      </footer>
    </div>
  );
}
